////////////////////////////////////////////////////////////////////////////////
    ...
    function init() {
        ...
        materialColor.setRGB(1.0, 1.0, 0);
    ...
    function createNewTeapot() {
        ...
        var body = new THREE.Mesh(new THREE.SphereGeometry(150, 20, 10), shading === "wireframe" ? wireMaterial : (
            shading === "flat" ? flatMaterial : (
                shading === "smooth" ? gouraudMaterial : (
                    shading === "glossy" ? phongMaterial : (
                        shading === "textured" ? texturedMaterial : reflectiveMaterial)))));

        var head = new THREE.Mesh(new THREE.SphereGeometry(90, 20, 10), shading === "wireframe" ? wireMaterial : (
            shading === "flat" ? flatMaterial : (
                shading === "smooth" ? gouraudMaterial : (
                    shading === "glossy" ? phongMaterial : (
                        shading === "textured" ? texturedMaterial : reflectiveMaterial)))));
        head.position.set(0, 140, 120);

        var billShape = new THREE.BoxBufferGeometry(100, 50, 200, 10, 10, 100)
        var billColor = new THREE.MeshBasicMaterial({
            color: 0xCC6600
        });
        var bill = new THREE.Mesh(billShape, billColor)
        bill.position.set(0, 130, 180)

        var eye = new THREE.SphereGeometry(25, 20, 10)
        var eyeColor = new THREE.MeshBasicMaterial({
        	color: 0x000000,
        	shading: "glossy"
        })
        var rightEye = new THREE.Mesh(eye, eyeColor)
        rightEye.position.set(40, 180, 170)
        var leftEye = new THREE.Mesh(eye, eyeColor)
        leftEye.position.set(-40, 180, 170)
        // scene.add( teapot );
        scene.add(body);
        scene.add(head);
        scene.add(bill);
